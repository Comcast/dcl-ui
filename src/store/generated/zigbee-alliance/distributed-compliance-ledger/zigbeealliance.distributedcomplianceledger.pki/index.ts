...
export default {
	...
	getters: {
		...
		getApprovedCertificatesBySubjectKeyId: (state) => (params = { params: {} }) => {
			if (!(<any>params).query) {
				(<any>params).query = null
			}
			return state.ApprovedCertificatesBySubjectKeyId[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryApprovedCertificatesBySubjectKeyId({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
			try {
				const key = params ?? {};
				const queryClient = await initQueryClient(rootGetters)
				let value = (await queryClient.queryApprovedCertificatesBySubjectKeyId(key.subjectKeyId)).data
				commit('QUERY', { query: 'ApprovedCertificatesBySubjectKeyId', key: { params: { ...key }, query }, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryApprovedCertificatesBySubjectKeyId', payload: { options: { all }, params: { ...key }, query } })
				return getters['getApprovedCertificatesBySubjectKeyId']({ params: { ...key }, query }) ?? {}
			} catch (e) {
				throw new SpVuexError('QueryClient:QueryApprovedCertificatesBySubjectKeyId', 'API Node Unavailable. Could not perform query: ' + e.message)
			}
		},
		...
		async QueryNocCertificates({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
			try {
				const key = params ?? {};
				const queryClient = await initQueryClient(rootGetters)
				let value = (await queryClient.queryNocCertificates(key.vid, key.pid)).data
		...
		async QueryNocIcaCertificates({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
			try {
				const key = params ?? {};
				const queryClient = await initQueryClient(rootGetters)
				let value = (await queryClient.queryNocIcaCertificates(key.vid, key.pid)).data
		...
		async QueryRevokedNocRootCertificates({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
			try {
				const key = params ?? {};
				const queryClient = await initQueryClient(rootGetters)
				let value = (await queryClient.queryRevokedNocRootCertificates(key.vid)).data
