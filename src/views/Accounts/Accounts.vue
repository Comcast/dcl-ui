...
import ProposeNewAccount from "./ProposeNewAccount";
import GrantActionAccount from "./GrantActionAccount";
    ...
    <TabView>
      <TabPanel header="All Active Accounts">
        <Button
          ...
          label="Propose-Account"
          >Propose Account</Button
        >
        <div class="mb-4"></div>
        <DataTable
          :value="allActiveAccounts"
          :auto-layout="true"
          :paginator="true"
          :rows="10"
        ...
        >
          ...
          <Column
            field="base_account.account_number"
          ...
          >
					<template #body="{ data }">
						...
						<span class="mt-3  pi pi-copy p-button-copy" v-tooltip.bottom="'copy public key'"></span></button>
          ...
          >
            <template #body="{ data }">
              <Button
                label="Revoke"
                @click="
                  showGrantActionAccountDialog(data, 'ProposeRevokeAccount')
                "
                iconPos="left"
                icon="pi pi-ban"
        ...
        >
          ...
          <Column field="account.roles" header="Roles">
            <template #body="row">
              <ul>
                <li
                  v-for="(role, index) in row.data.account.roles"
                  :key="index"
                >
                  {{ role }}
                </li>
          ...
          <Column field="account.approvals" header="Approvals">
            <template #body="row">
              <ol>
                <li
                  class="mb-2"
                  v-for="(approval, index) in row.data.account.approvals"
                  :key="index"
                >
                  Address : {{ trimAddress(approval.address) }} <br />
                  Time : {{ new Date(approval.time * 1000).toString() }} <br />
            ...
            <template #body="row">
              <ol>
                <li
                  class="mb-2"
                  v-for="(reject, index) in row.data.account.rejects"
                  :key="index"
                >
          ...
          >
            <template #body="{ data }">
              <Button
                label="Revoke"
                @click="
                  showGrantActionAccountDialog(data, 'ApproveRevokeAccount')
                "
                iconPos="left"
                icon="pi pi-ban"
                class="p-button-danger"
