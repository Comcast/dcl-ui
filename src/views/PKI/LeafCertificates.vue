...
export default {
	...
	computed: {
		...
		allChildCertificates() {
			...
			let childCerts = [];
			const certificatesArray = this.$store.getters[
				"zigbeealliance.distributedcomplianceledger.pki/getChildCertificates"
			]({
				params: {
					issuer: this.subject,
			...
			if (certificatesArray && certificatesArray.childCertificates) {
				...
				childCerts = childCerts.map((childCert) => {
					let cert = nonRootCertificates.find(
						(item) => item.subjectKeyId == childCert.subjectKeyId
					);
	...
	created: function () {
		...
		this.$store
			.dispatch(
				"zigbeealliance.distributedcomplianceledger.pki/QueryChildCertificates",
				{
					options: {
						subscribe: false,
						all: true,
...
<template>
	<div class="prime-vue-container">
		<div v-if="allChildCertificates.length > 0">
			<DataTable v-model:expandedRows="expandedRows" :value="allChildCertificates" :auto-layout="true" showGridlines
				stripedRows>
				...
				<Column headerStyle="width: 4rem; text-align: center" bodyStyle="text-align: center; overflow: visible">
					<template #body="{ data }">
						<Button label="Revoke" class="p-button-danger" @click="showRevokeCertificateDialog(data)" iconPos="left"
							icon="pi pi-ban" v-bind:class="{ 'p-disabled': !isSignedIn }"></Button>

						<Button label="Download" class="p-button-success mt-1" icon="pi pi-download"
							@click="downloadCertificateChain(data)"></Button>
					</template>
				</Column>
			...
			<Dialog header="Revoke X509 Certificate" @update:visible="dismissRevokeCertificateDialog"
				:visible="showRevokeCert" :style="{ width: '50vw' }" class="p-fluid" :modal="true">
				<GrantActionRootCertificate action="RevokeX509Cert" :certificate="revokeCertificate"
					@close-dialog="dismissRevokeCertificateDialog"></GrantActionRootCertificate>
			</Dialog>
		</div>
		<div v-else>
