...
export default {
  ...
  methods: {
    ...
    deleteModel(model) {
      ...
      this.$store
        .dispatch(
          `zigbeealliance.distributedcomplianceledger.model/sendMsgDeleteModel`,
          {
            value: {
...
<template>
  <div class="prime-vue-container ml-5">
      ...
      >
        <template #body="{ data }">
          <span style="margin-right: 0.1rem">
            <Button
              label=""
              @click="showModelInfoDialog(data, true)"
              iconPos="left"
              icon="pi pi-info-circle"
              class="
                p-button-rounded p-button-primary p-button-text p-button-info
              "
              v-tooltip="'Show Model'"
            />
          </span>
          <span style="margin-right: 0.1rem">
            <Button
              label=""
              @click="showModelInfoDialog(data, false)"
              iconPos="left"
              icon="pi pi-pencil"
              class="p-button-rounded p-button-secondary p-button-text"
              v-bind:class="{ 'p-disabled': !isSignedIn }"
              v-tooltip="'Update Model'"
            />
          </span>
          <span style="margin-right: 0.1rem">
            <Button
              label=""
              @click="confirmDeleteModel(data)"
              iconPos="left"
              icon="pi pi-trash"
    ...
    <Dialog
      ...
      @update:visible="dismissModelInfoDialog"
      :style="{ width: '40vw' }"
      :header="viewOnly ? 'Device Model' : 'Device Model Add/Update'"
      :modal="true"
    >
      <ModelInfo
        :viewOnly="viewOnly"
        :selectedModel="selectedModelInfo"
        @close-dialog="dismissModelInfoDialog"
      ></ModelInfo>
    </Dialog>
  </div>
</template>
