					...
					<div>
						...
						<span class="bold transition" :class="{ scaleBig: scaleClass }">
							<vue3-autocounter
								...
								:startAmount="0"
								:endAmount="
									lastBlockParticipatingNodes == 0
										? 0
										: (lastBlockParticipatingNodes.length /
												(validatorCount - jailedValidatorCount)) *
										  100
								"
								:duration="1.2"
								prefix=""
...
export default {
	...
	computed: {
		...
		lastBlockParticipatingNodes() {
			...
			if (
				this.$store.state.common.blocks.blocks &&
				this.$store.state.common.blocks.blocks.length > 0
			) {
